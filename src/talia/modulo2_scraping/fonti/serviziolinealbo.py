"""Spider generico per albi pretori su piattaforma "ServiziOnLine" (ASP.NET + DevExpress).

Stessa piattaforma di Agrigento (`agrigento.py`, dedicato), riusata qui in forma
parametrica dopo aver trovato lo stesso URL/struttura DOM su Pachino e Barrafranca
(esplorazione manuale, 2026-08-06): `<base_url>/ServiziOnLine/AlboPretorio/AlboPretorio`,
contenuto iniettato via callback DevExpress dopo networkidle. Richiede Playwright.

Struttura DOM (dopo rendering Playwright):
  - Ogni atto: <span class="text text-custom p-1">{numero}/{anno} del {data} - {resto}
               <em>{oggetto}</em></span>
  - {resto} varia per tenant: a volte include un riferimento settoriale finale
    "- {numero_settoriale}/{anno} del {data_settoriale}" (visto su Barrafranca), a
    volte no (visto su Pachino) — il parsing tollera entrambi i casi.
  - Permalink: attributo data-link="http://...?anno=YYYY&numero=NNN"
  - Paginazione: bottoni PBN (DevExpress) — cliccabili con Playwright

A differenza di Agrigento, la pagina raggiungibile via URL diretto non richiede il
passaggio dal catalogo servizi (`/ServiziOnLine/Istanze/landingIstanza?Id=N`) — la
navigazione diretta a `/ServiziOnLine/AlboPretorio/AlboPretorio` è sufficiente.

Non ancora consolidato con `agrigento.py` (stessa piattaforma, codice duplicato):
lasciato separato per non toccare uno scraper già verificato in produzione senza
necessità concreta. Da valutare un refactor comune se emergono altri tenant.
...
def _estrai_tipo(testo: str) -> str:
    """Isola il tipo atto dal testo tra la data e l'oggetto, scartando l'eventuale
    riferimento settoriale finale ("- NNNN/YYYY del DD/MM/YYYY", visto su alcuni tenant
    come Barrafranca, assente su altri come Pachino)."""
    m = _RE_RESTO_CON_SETTORIALE.match(testo)
    return m.group(1).strip() if m else testo.strip()


def _parse_html(html: str, codice_istat: str) -> list[AttoMetadato]:
    """Estrae atti dal DOM già renderizzato (da Playwright o HTML fixture).

    Deduplicazione per url_fonte: un atto con lo stesso permalink viene emesso una
    sola volta. Stesso approccio di `agrigento.py` (permalink-first, poi lo span del
    list-item corrispondente ancorato via `data-bs-target="#collapse{numero}_{anno}"`).
    """
    ...
    for plink_m in _RE_PERMALINK.finditer(html):
        ...
        span_m = re.search(
            ...
            html[max(0, target_idx - 200) : target_idx + 4000],
            re.DOTALL,
        )
        if not span_m:
            continue
...
def scarica_atti(
    ...
    *,
    ...
    """Scarica atti con Playwright da un albo pretorio "ServiziOnLine" (ASP.NET/DevExpress).
...
def prepara_ente(
    ...
    """Upsert dell'ente nel DB (prerequisito per inserisci_atto)."""
    upsert_ente(
        conn,
        EnteMetadato(denominazione=denominazione, codice_istat=codice_istat, provincia=provincia),
    )
