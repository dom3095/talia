...
|----|--------|-------|------|
| [TAL-63](TAL-63.md) | jCityGov: `url_fonte` mai stato un permalink funzionante | 🕷️ SCR | P0, esplicabilità; segnalato da Dom cliccando un link reale; verificato con Playwright (freddo e con sessione, curl puro): il formato `mostraDettaglio` dà sempre errore server-side; trovato il formato vero (`/display/<id>`) cliccando davvero il bottone del portale; codice corretto; backfill di prova su Acate (41 atti) fatto e verificato, ma ha rivelato che `red_flags.atti_cig` ha una copia propria dell'URL non sincronizzata — corretta anche quella per Acate; **backfill completo (85.435 atti + red_flags) non ancora eseguito** (in attesa di conferma Dom) |
